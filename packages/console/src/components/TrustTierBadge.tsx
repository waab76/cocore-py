...
const CONFIDENTIAL_TITLE =
  "Confidential tier (experimental). A trusted brokerage routed this job to genuine Apple hardware running the measured, signed agent under a hardened runtime, with an enclave-held signing and decryption key the operator can't copy off the machine, and countersigned the receipt to prove it. It aims to keep prompts unreadable to the operator — a raised bar, not a hardware guarantee: Apple Silicon has an enclave for keys but not for general compute, so a compromised kernel or physical access could still expose the prompt. Don't send anything you'd need cryptographically kept private.";
